...
puts "DEAL: #{cardName dupeHand.pop}"
total = cycle hand
livePlayer = [total, autoHand]
end


def livePlayerHit livePlayer
autoHand = livePlayer.pop
total = livePlayer.pop
hitChoice = 'h'

if total < 22 && hitChoice == 'h'
	puts
	...
	if hitChoice == 'h'
		newCard = $deck.pop
		autoHand.unshift newCard
		total = cycle autoHand
		puts "VALUE: #{total}"
...
#autoPlayer = [total, autoHand]
#autoPlayer = [players, [total, autoHand]]


#deal
while players > 0
	
	autoPlayer players  #loads all automated hands in hash

	dealt = autoPlayer[:players]
	dealtHand = dealt.pop
	dealtTotal = dealt.pop
	...
	puts "DEAL: #{cardName dealtHand.pop}"
	puts "DEAL: --"
	dealtHand.pop
	players = players - 1
end
